  ...
  module Internal
    module Transport
      ...
      class BaseClient
        ...
        #
        #   @option req [Class<FinchAPI::Internal::Type::BasePage>, nil] :page
        #
        #   @option req [Class<FinchAPI::Internal::Type::BaseStream>, nil] :stream
        #
        ...
        private def build_request(req, opts)
          ...
          path = FinchAPI::Internal::Util.interpolate_path(uninterpolated_path)

          query = FinchAPI::Internal::Util.deep_merge(req[:query].to_h, opts[:extra_query].to_h)
        ...
        def send_request(request, redirect_count:, retry_count:, send_retry_header:)
          ...
          headers = FinchAPI::Internal::Util.normalized_headers(response&.each_header&.to_h)
          ...
          in 300..399 if redirect_count >= self.class::MAX_REDIRECTS
            ...
            raise FinchAPI::Errors::APIConnectionError.new(url: url, response: response, message: message)
          in 300..399
            self.class.reap_connection!(status, stream: stream)
          ...
          in (400..) if retry_count >= max_retries || !self.class.should_retry?(status, headers: headers)
            decoded = Kernel.then do
              FinchAPI::Internal::Util.decode_content(headers, stream: stream, suppress_error: true)
            ensure
              self.class.reap_connection!(status, stream: stream)
            end
        ...
        #
        # @param page [Class<FinchAPI::Internal::Type::BasePage>, nil]
        #
        ...
        def request(req)
          ...
          headers = FinchAPI::Internal::Util.normalized_headers(response.each_header.to_h)
          decoded = FinchAPI::Internal::Util.decode_content(headers, stream: stream)
          case req
          in {stream: Class => st}
            st.new(
              model: model,
