...
class FinchAPI::Test::UtilDataHandlingTest < Minitest::Test
  def test_left_map
    assert_pattern do
      FinchAPI::Internal::Util.deep_merge({a: 1}, nil) => nil
    end
  end

  def test_right_map
    assert_pattern do
      FinchAPI::Internal::Util.deep_merge(nil, {a: 1}) => {a: 1}
    end
  end

  def test_disjoint_maps
    assert_pattern do
      FinchAPI::Internal::Util.deep_merge({b: 2}, {a: 1}) => {a: 1, b: 2}
    end
  end

  def test_overlapping_maps
    assert_pattern do
      FinchAPI::Internal::Util.deep_merge({b: 2, c: 3}, {a: 1, c: 4}) => {a: 1, b: 2, c: 4}
    end
  end

  def test_nested
    assert_pattern do
      FinchAPI::Internal::Util.deep_merge({b: {b2: 1}}, {b: {b2: 2}}) => {b: {b2: 2}}
    end
  end

  def test_nested_left_map
    assert_pattern do
      FinchAPI::Internal::Util.deep_merge({b: {b2: 1}}, {b: 6}) => {b: 6}
    end
  end

  def test_omission
    merged = FinchAPI::Internal::Util.deep_merge(
      {b: {b2: 1, b3: {c: 4, d: 5}}},
      {b: {b2: 1, b3: {c: FinchAPI::Internal::OMIT, d: 5}}}
    )
  ...
  def test_dig
    assert_pattern do
      FinchAPI::Internal::Util.dig(1, nil) => 1
      FinchAPI::Internal::Util.dig({a: 1}, :b) => nil
      FinchAPI::Internal::Util.dig({a: 1}, :a) => 1
      FinchAPI::Internal::Util.dig({a: {b: 1}}, [:a, :b]) => 1

      FinchAPI::Internal::Util.dig([], 1) => nil
      FinchAPI::Internal::Util.dig([nil, [nil, 1]], [1, 1]) => 1
      FinchAPI::Internal::Util.dig({a: [nil, 1]}, [:a, 1]) => 1
...
class FinchAPI::Test::UtilUriHandlingTest < Minitest::Test
  def test_parsing
    %w[
      http://example.com
      https://example.com/
      ...
      parsed = FinchAPI::Internal::Util.parse_uri(url)
      unparsed = FinchAPI::Internal::Util.unparse_uri(parsed).to_s

      assert_equal(url, unparsed)
      assert_equal(parsed, FinchAPI::Internal::Util.parse_uri(unparsed))
    end
  end
  ...
  def test_joining
    cases = [
      [
        "h://a.b/c?d=e",
        "h://nope/ignored",
        FinchAPI::Internal::Util.parse_uri("h://a.b/c?d=e")
      ],
      [
        "h://a.b/c?d=e",
        "h://nope",
    ...
    cases.each do |expect, lhs, rhs|
      assert_equal(
        URI.parse(expect),
        FinchAPI::Internal::Util.join_parsed_uri(
          FinchAPI::Internal::Util.parse_uri(lhs),
          rhs
        )
      )
    end
  ...
  def test_joining_queries
    ...
    cases.each do |path, expected|
      assert_equal(
        URI.parse(expected),
        FinchAPI::Internal::Util.join_parsed_uri(
          FinchAPI::Internal::Util.parse_uri(base_url),
          {path: path}
        )
      )
    end
...
class FinchAPI::Test::RegexMatchTest < Minitest::Test
  ...
  def test_jsonl_content
    ...
    cases.each do |header, verdict|
      assert_pattern do
        FinchAPI::Internal::Util::JSONL_CONTENT.match?(header) => ^verdict
      end
    end
  end
end
...
class FinchAPI::Test::UtilFormDataEncodingTest < Minitest::Test
  class FakeCGI < CGI
    def initialize(headers, io)
      ...
      @io = FinchAPI::Internal::Util::ReadIOAdapter.new(encoded.to_enum) {}
  ...
  def test_encoding_length
    headers, = FinchAPI::Internal::Util.encode_content(
  ...
  def test_file_encode
    ...
    fileinput = FinchAPI::Internal::Type::Converter.dump(FinchAPI::Internal::Type::FileInput, "abc")
    ...
    cases = {
      ...
      file => [file.basename.to_path, /^class FinchAPI/],
      FinchAPI::FilePart.new(file, filename: "d o g") => ["d%20o%20g", /^class FinchAPI/]
    }
  ...
  def test_hash_encode
    ...
    cases = {
      ...
      {pathname: FinchAPI::FilePart.new(Pathname(__FILE__))} => {"pathname" => -> { _1.read in /^class FinchAPI/ }}
    }
    cases.each do |body, testcase|
      encoded = FinchAPI::Internal::Util.encode_content(headers, body)
      cgi = FakeCGI.new(*encoded)
...
class FinchAPI::Test::UtilIOAdapterTest < Minitest::Test
  def test_copy_read
    cases = {
      StringIO.new("abc") => "abc",
    ...
    cases.each do |input, expected|
      io = StringIO.new
      # rubocop:disable Lint/EmptyBlock
      adapter = FinchAPI::Internal::Util::ReadIOAdapter.new(input) {}
      # rubocop:enable Lint/EmptyBlock
  ...
  def test_copy_write
    ...
    cases.each do |input, expected|
      enum = FinchAPI::Internal::Util.writable_enum do |y|
...
class FinchAPI::Test::UtilFusedEnumTest < Minitest::Test
  def test_rewind_closing
    ...
    fused = FinchAPI::Internal::Util.fused_enum(enum, external: true) do
      touched = true
      loop { enum.next }
    end
  ...
  def test_thread_interrupts
    ...
    fused_1 = FinchAPI::Internal::Util.fused_enum(enum, external: true) { loop { enum.next } }
    fused_2 = FinchAPI::Internal::Util.chain_fused(fused_1) { fused_1.each(&_1) }
    fused_3 = FinchAPI::Internal::Util.chain_fused(fused_2) { fused_2.each(&_1) }
  ...
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = FinchAPI::Internal::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_rewind_chain
    once = 0
    fused = FinchAPI::Internal::Util.fused_enum([1, 2, 3].to_enum) do
      once = once.succ
    end
      .lazy
      .map(&:succ)
  ...
  def test_external_iteration
    iter = [1, 2, 3].to_enum
    first = iter.next
    fused = FinchAPI::Internal::Util.fused_enum(iter, external: true)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = FinchAPI::Internal::Util.fused_enum(enum)
    first = fused.next
  ...
  def test_close_fused_sse_chain
    ...
    fused_1 = FinchAPI::Internal::Util.fused_enum(enum)
    fused_2 = FinchAPI::Internal::Util.decode_lines(fused_1)
    fused_3 = FinchAPI::Internal::Util.decode_sse(fused_2)

    assert_equal(0, taken)
    FinchAPI::Internal::Util.close_fused!(fused_3)
    assert_equal(0, taken)
  end
end
...
class FinchAPI::Test::UtilSseTest < Minitest::Test
  def test_decode_lines
    ...
    cases.each do |enum, expected|
      eols.each do |eol|
        lines = FinchAPI::Internal::Util.decode_lines(enum.map { _1.gsub("\n", eol) })
  ...
  def test_mixed_decode_lines
    ...
    cases.each do |enum, expected|
      lines = FinchAPI::Internal::Util.decode_lines(enum)
  ...
  def test_decode_sse
    ...
    cases.each do |name, test_cases|
      test_cases.each do |input, expected|
        actual = FinchAPI::Internal::Util.decode_sse(input).map(&:compact)
