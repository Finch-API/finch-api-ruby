...
class FinchAPI::Test::UtilDataHandlingTest < Minitest::Test
  def test_left_map
    assert_pattern do
      FinchAPI::Util.deep_merge({a: 1}, nil) => nil
    end
  end

  def test_right_map
    assert_pattern do
      FinchAPI::Util.deep_merge(nil, {a: 1}) => {a: 1}
    end
  end

  def test_disjoint_maps
    assert_pattern do
      FinchAPI::Util.deep_merge({b: 2}, {a: 1}) => {a: 1, b: 2}
    end
  end

  def test_overlapping_maps
    assert_pattern do
      FinchAPI::Util.deep_merge({b: 2, c: 3}, {a: 1, c: 4}) => {a: 1, b: 2, c: 4}
    end
  end

  def test_nested
    assert_pattern do
      FinchAPI::Util.deep_merge({b: {b2: 1}}, {b: {b2: 2}}) => {b: {b2: 2}}
    end
  end

  def test_nested_left_map
    assert_pattern do
      FinchAPI::Util.deep_merge({b: {b2: 1}}, {b: 6}) => {b: 6}
    end
  end

  def test_omission
    merged = FinchAPI::Util.deep_merge(
      {b: {b2: 1, b3: {c: 4, d: 5}}},
      {b: {b2: 1, b3: {c: FinchAPI::Util::OMIT, d: 5}}}
    )
  ...
  def test_dig
    assert_pattern do
      FinchAPI::Util.dig(1, nil) => 1
      FinchAPI::Util.dig({a: 1}, :b) => nil
      FinchAPI::Util.dig({a: 1}, :a) => 1
      FinchAPI::Util.dig({a: {b: 1}}, [:a, :b]) => 1

      FinchAPI::Util.dig([], 1) => nil
      FinchAPI::Util.dig([nil, [nil, 1]], [1, 1]) => 1
      FinchAPI::Util.dig({a: [nil, 1]}, [:a, 1]) => 1
      FinchAPI::Util.dig([], 1.0) => nil

      FinchAPI::Util.dig(Object, 1) => nil
      FinchAPI::Util.dig([], 1.0, 2) => 2
      FinchAPI::Util.dig([], 1.0) { 2 } => 2
    end
  end
end
...
class FinchAPI::Test::UtilUriHandlingTest < Minitest::Test
  def test_parsing
    %w[
      ...
      parsed = FinchAPI::Util.parse_uri(url)
      unparsed = FinchAPI::Util.unparse_uri(parsed).to_s

      assert_equal(url, unparsed)
      assert_equal(parsed, FinchAPI::Util.parse_uri(unparsed))
    end
  end
  ...
  def test_joining
    cases = [
      [
        "h://a.b/c?d=e",
        "h://nope/ignored",
        FinchAPI::Util.parse_uri("h://a.b/c?d=e")
      ],
      [
        "h://a.b/c?d=e",
        "h://nope",
    ...
    cases.each do |expect, lhs, rhs|
      assert_equal(
        URI.parse(expect),
        FinchAPI::Util.join_parsed_uri(
          FinchAPI::Util.parse_uri(lhs),
          rhs
        )
      )
    end
  ...
  def test_joining_queries
    ...
    cases.each do |path, expected|
      assert_equal(
        URI.parse(expected),
        FinchAPI::Util.join_parsed_uri(
          FinchAPI::Util.parse_uri(base_url),
          {path: path}
        )
      )
    end
...
class FinchAPI::Test::UtilFormDataEncodingTest < Minitest::Test
  class FakeCGI < CGI
    def initialize(headers, io)
      ...
      @io = FinchAPI::Util::ReadIOAdapter.new(io) {}
  ...
  def test_file_encode
    ...
    cases.each do |body, val|
      encoded = FinchAPI::Util.encode_content(headers, body)
...
class FinchAPI::Test::UtilIOAdapterTest < Minitest::Test
  def test_copy_read
    ...
    cases.each do |input, expected|
      io = StringIO.new
      # rubocop:disable Lint/EmptyBlock
      adapter = FinchAPI::Util::ReadIOAdapter.new(input) {}
      # rubocop:enable Lint/EmptyBlock
  ...
  def test_copy_write
    ...
    cases.each do |input, expected|
      enum = FinchAPI::Util.writable_enum do |y|
...
class FinchAPI::Test::UtilFusedEnumTest < Minitest::Test
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = FinchAPI::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_rewind_chain
    once = 0
    fused = FinchAPI::Util.fused_enum([1, 2, 3].to_enum) do
      once = once.succ
    end
      .lazy
      .map(&:succ)
  ...
  def test_external_iteration
    it = [1, 2, 3].to_enum
    first = it.next
    fused = FinchAPI::Util.fused_enum(it, external: true)
  ...
  def test_close_fused
    ...
    FinchAPI::Util.close_fused!(fused)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = FinchAPI::Util.fused_enum(enum)
    first = fused.next
